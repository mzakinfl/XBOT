	...
	if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
        ...
        switch (command) {
            ...
            case 'daftar':
                ...
                if(isNaN(umurUser)) return await bocchi.reply(from, 'Umur Harus Berupa Angka !', id)
                if (Number(umurUser) >= 30) return await bocchi.reply(from, ind.ageOld(), id)
                if (Number(umurUser) < 16) return await bocchi.reply(from, 'Kamu Belum Cukup Umur Dek', id)
       ...
       leaderboard += `${nom}. wa.me/${_level[i].id.replace('@c.us', '')}\n➸ *XP*: ${_level[i].xp} *Level*: ${_level[i].level}\n➸ *Role*: ${roles}\n\n`
            ...
            .catch(async (err) => {
            console.error(err)
            await bocchi.reply(from, `Ga Support Url Yang Itu, Contoh: https://youtu.be/i4EJzIcnWVg`, id)
                    })
            break
            case 'ytmp4': //XBOT Ini WM Ngentot
            ...
            .catch(async (err) => {
            console.error(err)
            await bocchi.reply(from, `Not Supported Url`, id)
                    })
            break
            case 'tiktokpic':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'tiktoknowm': // by: VideFrelan
                case 'tktnowm':
                    ...
                    if (!isUrl(url) && !url.includes('tiktok.com')) return bocchi.reply(from, `URL Bukan dari Tiktok!`, id)
                    await bocchi.reply(from, ind.wait(pushname), id)
                ...
                case 'tiktok':
                    ...
                    downloader.tik(url)
                        .then(async ({ result })=> {
                            ...
                            await fs.writeFile(`./temp/${sender.id}.mp4`, buffer)
                            await bocchi.sendFile(from, `./temp/${sender.id}.mp4`, `TikTokNoWM.mp4`, '', id)
                            console.log('Success sending tiktok MP4!')
                            fs.unlinkSync(`./temp/${sender.id}.mp4`)
                        })
                        .catch(async (err) => {
                            console.log(err)
                ...
                misc.its(q)
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.story.itemlist.length; i++) {
                            ...
                            await bocchi.sendFileFromUrl(from, urlDownload, '', 'By: XBot', id)
            ...
            break
            case 'brainly': //By: XBot
                if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                fun.brainly(q)
                .then(async ( { result }) => {
                    await bocchi.reply(from, `➸ *Jawaban*: ${result.data}`, id)
                  })
                  .then(() => {
                      console.log('Sukses mengirim jawaban brainly!')
                  })
                  .catch(async (err) => {
                      console.error(err)
              ...
              break
                  case 'randompantun': //By: XBot
                  if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                  await bocchi.reply(from, ind.wait(pushname), id)
                  fun.pantun()
                  .then(async ( { result }) => {
                      await bocchi.reply(from, `*Pantun*: ${result.pantun}`, id)
                    })
                    .then(() => {
                        console.log('Sukses mengirim random pantun')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!')
                ...
                break
                case 'randomquotesid': //By: XBot
                    if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                    await bocchi.reply(from, ind.wait(pushname), id)
                    fun.quotesid()
                    .then(async ( { result }) => {
                        await bocchi.reply(from, `*Quotes ID*: ${result.kata}`, id)
                      })
                      .then(() => {
                          console.log('Sukses mengirim random quotes EN')
                      })
                      .catch(async (err) => {
                          console.error(err)
                          await bocchi.reply(from, 'Error!')
                  ...
                  break
                  case 'randomquotes': //By: XBot
                      if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                      await bocchi.reply(from, ind.wait(pushname), id)
                      fun.quotesen()
                      .then(async ( { result }) => {
                          await bocchi.reply(from, `*Quotes EN*: ${result.tags}\n ${result.content}\n *Author*: ${result.author}`, id)
                        })
                        .then(() => {
                            console.log('Sukses mengirim random quotes EN')
                        })
                    ...
                    case 'asci': //By: XBot
                        ...
                        fun.asci()
                        .then(async ( { result }) => {
                            await bocchi.reply(from, `${result.text}`, id)
                          })
                          .then(() => {
                              console.log('Sukses mengirim text asci')
                          })
                          .catch(async (err) => {
                              console.error(err)
                              await bocchi.reply(from, 'Error!')
                      ...
                      break
                      case 'pinterestdl': //By: XBot
                          if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                          await bocchi.reply(from, ind.wait(pushname), id)
                          misc.pinterestdl(url)
                          .then(async ( { result }) => {
                          await bocchi.reply(from, `Url: ${result.fileUrl}\n Title: ${result.title}\n Type: ${result.type}`, id)
                            })
                            .then(() => {
                                console.log('Sukses mengirim pinterestdl')
                            })
                            .catch(async (err) => {
                                console.error(err)
                                await bocchi.reply(from, 'Error!')
                        ...
                        case 'cersex': //By: XBot
                            ...
                            fun.cersex()
                            .then(async ( { result }) => {
                              await bocchi.sendFileFromUrl(from, result.image, 'cersex.jpg', ``, id)
                              await bocchi.reply(from, `── *「 ${result.judul} 」* ──\n\n*Cerita:* ${result.cerita}`, id)
                              })
                              .then(() => {
                                  console.log('Sukses mengirim cersex')
                              })
                              .catch(async (err) => {
                                  console.error(err)
                                  await bocchi.reply(from, 'Error!')
	            ...
	            if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                   await bocchi.reply(from, ind.wait(pushname), id)
                  	axios.get(`https://api.vhtear.com/randomloli&apikey=${config.vhtear}`)
	                 	.then(async (res) => {
    	               	await bocchi.sendFileFromUrl(from, res.data.result.result, 'loli.jpg', '', id)
                     	})
	                 	.catch(async (err) => {
    	             	console.error(err)
      	          	await bocchi.reply(from, 'Error!', id)
	        ...
	        if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
          await bocchi.reply(from, ind.wait(pushname), id)
        	axios.get(`https://api.vhtear.com/pinterest?query=${q}&apikey=${config.vhtear}`)
	    	.then(async (res) => {
    	      	for (let i = 0; i < 5; i++) {
    		  	await bocchi.sendFileFromUrl(from, res.data.result[i], 'pin.jpg', '', id)
         		}
          	})
	         	.catch(async (err) => {
    	       	console.error(err)
                 ...
                 break
                        case 'puisigambar': // by XBot
                	if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                  await bocchi.reply(from, ind.wait(pushname), id)
    	  await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/puisi_image&apikey=${config.vhtear}`, 'puisi.jpg', '', id)
                 break
                    case 'prediksicuaca': //By: XBot
                        if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                        await bocchi.reply(from, ind.wait(pushname), id)
                        misc.cuaca(q)
                        .then(async ( { result }) => {
                            await bocchi.reply(from, `${result.weather}\n ${result.location}`, id)
                          })
                          .then(() => {
                              console.log('Sukses mengirim prediksi cuaca')
                          })
                          .catch(async (err) => {
                              console.error(err)
                              await bocchi.reply(from, 'Error!')
                      ...
                      break
                     case 'jadwaltvnow': //By: XBot
                          if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                          await bocchi.reply(from, ind.wait(pushname), id)
                          misc.jdwltvnow()
                          .then(async ( { result }) => {
                              await bocchi.reply(from, `➸ *Jam: ${result.jam}*\n ${result.jadwalTV}`, id)
                            })
                            .then(() => {
                                console.log('Sukses mengirim jadwaltv now')
                            })
                            .catch(async (err) => {
                                console.error(err)
                                await bocchi.reply(from, 'Error!')
                        ...
                        break
                        case 'jadwaltv': //By: XBot
                             if (!isRegistered) return  bocchi.reply(from, ind.notRegistered(), id)
                             await bocchi.reply(from, ind.wait(pushname), id)
                             misc.jdwltv(q)
                             .then(async ( { result }) => {
                               let list = '-----[ JADWAL LIST ]-----\n\n'
                                for (let i = 0; i < result.length; i++) {
                                   list += `${result[i]}\n\n`
                               }
                               await bocchi.reply(from, list, id)
                               console.log('Success Sending Jadwal List')
                           })
                           .catch(async (err) => {
                               console.error(err)
                               await bocchi.reply(from, 'Error!', id)
            ...
            case 'gempa':
                ...
                misc.bmkg()
                    ...
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Maintance', id)
                    })
            break
            case 'igstalk':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'help':
                ...
                if (args[0] === '1') {
                  ...
                  if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                    await bocchi.sendText(from, ind.menuMaker())
                } else if (args[0] === '4') {
                  if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                      await bocchi.sendText(from, ind.menuMedia())
                } else if (args[0] === '5') {
                  if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
            ...
            case 'igowner':
                ...
                await bocchi.reply(from, 'Ini IG Pacar Ku 🥰', id)
            break
            case 'intro':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.sendText(from, `-------➤INTRO
➤🙋‍♂Hallo Saya :V Saya Di Bangun 🛠️ Dengan Bahasa Python3.8 🐍 Dan Beberapa API🔥`)
            break
            case 'ping':
            case 'p':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'tos':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.sendLinkWithAutoPreview(from, '', ind.tos(ownerNumber))
            break
            case 'join':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                ...
                if (isOwner) {
                    ...
                    await bocchi.reply(from, ind.ok(pushname), id)
                    await bocchi.sendText(checkInvite.id, `Saya Telah Di Invite ${pushname}`)
                } else {
                    const getGroupData = await bocchi.getAllGroups()
                    if (getGroupData.length >= groupLimit) {
                        await bocchi.reply(from, `Grup Yang Ditampung XBot Sudah Penuh ${groupLimit}`, id)
                    } else if (getGroupData.size <= memberLimit) {
                        await bocchi.reply(from, `Najis Member Cuma Segini, Kalo Mau Undang Gua Member Nya Harus ${memberLimit} Orang`, id)
                    } else {
                        await bocchi.joinGroupViaLink(url)
            ...
            case 'asupan': // shansekai
                ...
                fun.asupan()
                    .then(async (body) => {
                        ...
                        await bocchi.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, 'asupan.mp4', 'Jangan Sange yah ^_^', id)
            ...
            case 'chika'://Piyobot
                ...
                fun.chika()
                    .then(async (body) => {
                        ...
                        await bocchi.sendFileFromUrl(from, `https://piyobot.000webhostapp.com//${chikax}.mp4`, 'asupan.mp4', 'Jangan Comly Asu.', id)
                        console.log('Success sending video!')
                    })
                    .catch(async (err) => {
                        console.error(err)
           ...
           break
            ...
            case 'ramalnama':
                ...
                if (args.length !== 1) return await bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                fun.ramalnama(args[0])
                    .then(async ({ result }) => {
                        if (result.status === 204) {
                            return await bocchi.reply(from, result.ramalan, id)
                        } else {
                         let ramalnama = `${result.hasil}`
                                  await bocchi.reply(from, ramalnama, id)
                                .then(() => console.log('Success sending ramal nama!'))
                        }
                    })
                    .catch(async (err) => {
                        console.error(err)
            ...
            case 'party':
                ...
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                console.log('Creating Party text...')
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/partytext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
            ...
            case 'water':
                ...
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/water_maker?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creting image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'metal':
                ...
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                console.log('Creating Metal text...')
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/metal_maker?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creting image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'thunder':
                ...
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/thundertext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'gamelogo':
                ...
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                console.log('Creating game logo text...')
            ...
            case 'silk':
                ...
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                console.log('Creating Silktext text...')
            ...
            case 'romantis':
                ...
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                console.log('Creating Romantis text...')
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/romancetext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creting image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'glow':
                ...
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/glowtext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creting image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'love':
                ...
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/lovemessagetext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creting image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            await bocchi.reply(from, ind.wait(pushname), id)
            console.log('Creating Gugel text...')
            const teks1gugel = q.substring(0, q.indexOf('|') - 1)
            const teks2gugel = q.substring(q.indexOf('|') + 2, q.lastIndexOf('|') - 1)
            const teks3gugel = q.substring(q.lastIndexOf('|') + 2)
            await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/googletext?text1=${teks1gugel}&text2=${teks2gugel}&text3=${teks3gugel}&apikey=${config.vhtear}`, id)
            console.log('Success!')
        break
            case 'padlock': // By: XBot
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
                console.log('Creating Padlock banner...')
                const teks1padlock = q.substring(0, q.indexOf('|') - 1)
                const teks2padlock = q.substring(q.lastIndexOf('|') + 2)
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/padlock?text1=${teks1padlock}&text2=${teks2padlock}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case 'ffbanner': // By: XBot
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return bocchi.reply(from, ind.wrongFormat(), id)
                await bocchi.reply(from, ind.wait(pushname), id)
            ...
            break
                case 'family100': //By: XBot
                  ...
                  await bocchi.reply(from, `➸ *Soal*: ${result.soal}`, id)
                      ...
                      .then(() => {
                          console.log('Sukses mengirim jawaban family!')
                      })
                      .catch(async (err) => {
                          console.error(err)
                          await bocchi.reply(from, 'Error!')
            ...
            case 'weton':
                ...
                fun.weton(tgl, bln, thn)
                    .then(async ({ result }) => {
                    await bocchi.reply(from, result.hasil, id)
                        .then(() => console.log('Succes sending weton info!'))
                })
                .catch(async (err) => {
                    console.error(err)
                    await bocchi.reply(from, 'Error!', id)
...
​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​​b`
                            ...
                            sharp(res)
                                ...
                                .toFile(`./temp/takestickstage_${sender.id}.webp`, async (err) => {
                                    ...
                                    if (fs.existsSync(`./temp/takestick_${sender.id}.webp`)) {
                                        ...
                                        const base64 = `data:image/webp;base64,${data.toString('base64')}`
                                        bocchi.sendRawWebpAsSticker(from, base64)
                                        console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
            ...
            case 'shopee':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                const nameBarang = q.substring(0, q.indexOf('|') - 1)
                const jumlehBarang = q.substring(q.lastIndexOf('|') + 2)
                await bocchi.reply(from, ind.wait(pushname), id)
                try {
                    fun.shopee(nameBarang, jumlehBarang)
                        .then(async ({ result }) => {
                            for (let i = 0; i < result.items.length; i++) {
                                const { nama, harga, terjual, shop_location, description, link_product, image_cover } = result.items[i]
                                await bocchi.sendFileFromUrl(from, image_cover, `${nama}.jpg`, ind.shopee(nama, harga, terjual, shop_location, description, link_product))
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif') {
                    ...
                    try {
                        ...
                        await bocchi.sendMp4AsSticker(from, videoBase64, { fps: 24, startTime: `00:00:00.0`, endTime : `00:00:05.0`, loop: 0 })
                            .then(async () => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                                await bocchi.sendText(from, ind.ok(pushname))
                            })
                    } catch (err) {
                        console.error(err)
            ...
            case 'xxxsearch':
                    ...
                    if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                    if (isGroupMsg) return await bocchi.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
                    if (!q) return await bocchi.reply(from, `Untuk mencari video dari XXX\ngunakan ${prefix}xxxsearch query\n\nContoh: ${prefix}xxxsearch japan`, id)
                    await bocchi.reply(from, ind.wait(pushname), id)
                    try {
                        nsfw.xxxsearch(q)
                            .then(async ({ result }) => {
                                let xxxsny = `...: XXX SEARCH :...`
                                for (let i = 0; i < result.data.length; i++) {
                                    xxxsny += `\n\n*Judul*: ${result.data[i].title}\n*Durasi*: ${result.data[i].duration}\n*URL*: ${result.data[i].url}\n\n=============`
                                }
                                await bocchi.sendFileFromUrl(from, result.data[0].image, 'XXXSEARCHVFBOT.jpg', xxxsny, id)
                                console.log('Success sending XXX Search!')
                            })
                    } catch (err) {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
                ...
                case 'xxxdl':
                    ...
                    if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                    if (isGroupMsg) return await bocchi.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
                    if (!q) return await bocchi.reply(from, `Untuk download video dari XXX\ngunakan ${prefix}xxxdl linkvideo`, id)
                    await bocchi.reply(from, ind.wait(pushname), id)
                    try {
                        nsfw.xxxdl(q)
                            .then(async ({ result }) => {
                                let xxxdl = `...: XXX DOWNLOADER :...`
                                for (let i = 0; i < result.urlVideo.length; i++) {
                                    xxxdl += `\n\n*Title*: ${result.title}\n*URL*: ${result.urlVideo[i].videoUrl}\n*Quality*: ${result.urlVideo[i].quality}\n\n=============`
                                }
                                await bocchi.sendFileFromUrl(from, result.urlVideo[2].videoUrl, 'XXXDLVFBOT.mp4', xxxdl, id)
                                console.log('Success sending XXX Video!')
                            })
                    } catch (err) {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'waifu18':
                ...
                if (isGroupMsg) {
                    ...
                    weeaboo.waifu(true)
                        .then(async ({ url }) => {
                            await bocchi.sendFileFromUrl(from, url, 'waifu.png', 'ngocok terus yaa', id)
                                .then(() => console.log('Success sending waifu!'))
                        })
                        .catch(async (err) => {
                            console.error(err)
            ...
            case 'kick':
                ...
                await bocchi.sendTextWithMentions(from, `Dadah Tod\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            ...
            case 'leave':
                ...
                if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return await bocchi.reply(from, ind.adminOnly(), id)
                await bocchi.sendText(from, 'Selamat Tinggal 👋')
            ...
            case 'shutdown':
                if (!isOwner) return await bocchi.reply(from, ind.ownerOnly(), id)
                await bocchi.sendText(from, 'Sampai Jumpa Lagi Kawan" 👋')
            ...
            default:
                if (isCmd) {
                    await bocchi.reply(from, ind.cmdNotFound(command, pushname), id)
                }
            break
            case 'give':
                if (!isOwner) return await bocchi.reply(from, ind.ownerOnly(), id)
